...
public:
    /**
     * http://www.lintcode.com/zh-cn/problem/word-ladder/-120-单词接龙
...
int main()
{
   ...
   /* dict.insert("hot");
    ...
    vector<string> nums(&A[0], &A[sizeof(A) / sizeof(str1)]);

    for (int i = 0; i < nums.size(); i++) {
        dict.insert(nums[i]);
    }

    cout << s.ladderLength(str1, str2, dict) << endl;
